...
class PolicyGapRetailReversal(Strategy):
    """
    Policy Gap Fade & Retail Reversal - V6 (QMT Bridge - Corrected)
    ...
    QMT Bridge specific notes:
    - Volume is in handlots (手) where 1 handlot = 100 shares
    - preClose column provides the actual previous day's close for accurate gap calculation
    - Volume ratios work correctly since both numerator and denominator are in same units
    ...
    def _select_stocks(self):
        ...
        for symbol in self.symbols:
            if symbol not in self.price_history:
                self.log_message(f"No price history for {symbol}")
                continue

            df = self.price_history[symbol]
            if len(df) < 22:
                self.log_message(f"Not enough data for {symbol}: {len(df)} bars")
                continue
            ...
            try:
                ...
                # Use close.iloc[-2] consistently - matches original strategy
                ...
                self.log_message(f"CANDIDATE {symbol}: gap={gap:.1%}, vol_ratio={vol_ratio:.2f}, sector={sector}")
...
if __name__ == "__main__":
    ...
    if not pandas_data:
        print("ERROR: No data loaded from QMT Bridge. Check your connection.")
        sys.exit(1)

    print(f"\nStarting backtest with QMT Bridge data")
    ...
    # Run backtest using the standard Strategy.backtest() method
    results = PolicyGapRetailReversal.backtest(
        PandasDataBacktesting,
        pd.to_datetime(backtesting_start_date),
        pd.to_datetime(backtesting_end_date),
        # benchmark_asset=None,  # Disable benchmark (Yahoo doesn't support A-shares)
